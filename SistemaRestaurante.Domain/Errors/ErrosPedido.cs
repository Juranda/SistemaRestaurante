...
public static class ErrosPedido
{
    ...
    public static Error AlterarParaStatusInvalido() => new Error("PED-DOM-003", "O item do pedido não pode ser alterado para este estado.", ErrorTypes.Conflict);
    public static Error PedidoSemItems() => new Error("PED-DOM-004", "O pedido não possui items.", ErrorTypes.Validation);
    public static Error AlterarStatusItemPedidoInexistente() => new Error("PED-DOM-005", "O pedido não possui este item. O status nao pode ser alterado", ErrorTypes.NotFound);
    public static Error PedidoNaoExiste() => new Error("PED-INF-001", "Pedido não existe", ErrorTypes.NotFound);
    public static Error ItemDoPedidoNaoExiste() => new Error("PED-INF-001", "Item do pedido não existe", ErrorTypes.NotFound);
}